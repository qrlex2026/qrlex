...
async function main() {
    ...
    for (const cat of categories) {
        ...
        const category = await prisma.category.create({
            data: { name: cat.name, restaurantId: rid, sortOrder: catOrder }
        });
        ...
        for (const prod of cat.products) {
            ...
            await prisma.product.create({
                data: {
                    ...
                    restaurantId: rid,
                    sortOrder: prodOrder,
                    isPopular: (prod as any).isPopular || false,
                }
            });
        }
        console.log(`  -> ${cat.products.length} products added`);
    }

    console.log("\n✅ Done! 10 categories, 60 products created.");
}
