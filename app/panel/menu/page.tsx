                                        ...
                                        {catProducts.map(p => (
                                            <div key={p.id} className="bg-white rounded-xl p-2.5 flex gap-2.5 shadow-sm border border-gray-100">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="mt-1">{p.discountPrice ? (<div className="flex items-center gap-1"><span className="text-xs font-bold text-emerald-600">₺{Number(p.discountPrice).toFixed(0)}</span><span className="text-[10px] text-gray-400 line-through">₺{Number(p.price).toFixed(0)}</span></div>) : (<span className="text-xs font-bold text-gray-900">₺{Number(p.price).toFixed(0)}</span>)}</div>
                                                </div>
