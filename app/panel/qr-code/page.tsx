...
export default function PanelQRCode() {
    ...
    const handleDownload = async (format: "png" | "svg") => {
        ...
        const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `qr-menu-${slug}.${format}`; a.click();
    };
    const handleCopy = () => { navigator.clipboard.writeText(menuUrl); setCopied(true); setTimeout(() => setCopied(false), 2000); };
    const handlePrint = () => { const w = window.open("", "_blank"); if (w) { w.document.write(`<html><body style="display:flex;justify-content:center;align-items:center;height:100vh;margin:0"><img src="${qrUrl}" /><script>setTimeout(()=>{window.print();window.close()},500)</script></body></html>`); } };

    if (sessionLoading || !slug) return <div className="text-center py-20 text-gray-500">Yükleniyor...</div>;

    return (
        <div className="space-y-6 max-w-3xl">
            <div><h1 className="text-2xl font-bold text-white">QR Kod Oluşturucu</h1><p className="text-sm text-gray-400 mt-1">Menünüz için QR kod oluşturun</p></div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 flex flex-col items-center gap-4">
                    ...
                    <div className="bg-white rounded-2xl p-4 shadow-xl">{qrUrl && <img src={qrUrl} alt="QR Menü" width={size > 300 ? 300 : size} height={size > 300 ? 300 : size} />}</div>
                    <p className="text-xs text-gray-500 text-center break-all">{menuUrl}</p>
                    <div className="flex gap-2 w-full">
                        <button onClick={() => handleDownload("png")} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium transition-colors"><Download size={16} /> PNG</button>
                ...
                <div className="space-y-4">
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                        <h3 className="text-sm font-semibold text-gray-300 flex items-center gap-2"><Palette size={16} className="text-emerald-400" /> Renk Teması</h3>
                        <div className="grid grid-cols-3 gap-2">{THEMES.map((theme) => (
                            <button key={theme.name} onClick={() => { setFgColor(theme.fg); setBgColor(theme.bg); }} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-medium transition-all border ${fgColor === theme.fg && bgColor === theme.bg ? "border-emerald-500 bg-emerald-500/10 text-emerald-300" : "border-gray-700 bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>
                                <div className="w-4 h-4 rounded-full border border-gray-600" style={{ background: `linear-gradient(135deg, #${theme.fg} 50%, #${theme.bg} 50%)` }} />{theme.name}
                            </button>
                        ))}</div>
                    </div>
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                        <h3 className="text-sm font-semibold text-gray-300 flex items-center gap-2"><Maximize size={16} className="text-emerald-400" /> Boyut</h3>
                        <input type="range" min={150} max={500} step={10} value={size} onChange={(e) => setSize(Number(e.target.value))} className="w-full accent-emerald-500" />
