...
export default function ReviewsManagement() {
    ...
    const renderStars = (rating: number) => (
        <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} size={14} className={s <= rating ? "text-amber-400" : "text-gray-700"} fill={s <= rating ? "currentColor" : "none"} />
            ))}
        </div>
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-gray-400 mt-1">{reviews.length} yorum</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                    ...
                    <p className="text-xs text-gray-500">Faydalı İşareti</p>
                </div>
            </div>
                    ...
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                            <div className="flex items-start justify-between">
                                ...
                                <button onClick={() => deleteReview(review.id)} className="p-2 rounded-lg hover:bg-red-500/10 transition-colors flex-shrink-0">
